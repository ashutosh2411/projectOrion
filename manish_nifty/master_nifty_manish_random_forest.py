...
def MAIN(file):
	...
	for i in range (10):
		if SPLIT_RANDOM == 'y':
			...
			X_train 	 = data[X_TRAIN_START + i*windoe_size : X_TRAIN_END + i*windoe_size           , X_COLS]
			Y_train 	 = data[X_TRAIN_START + i*windoe_size : X_TRAIN_END + i*windoe_size           , [Y_COLS,RETURNS_COLS]]
			X_test 		 = data[X_TEST_START  + i*windoe_size : X_TEST_END  + i*windoe_size           , X_COLS]
			Y_test 		 = data[X_TEST_START  + i*windoe_size : X_TEST_END  + i*windoe_size           , [Y_COLS,RETURNS_COLS]]	
			X_validation = data[X_TEST_END    + i*windoe_size : X_TEST_END  + i*windoe_size+test_size , X_COLS]
			Y_validation = data[X_TEST_END    + i*windoe_size : X_TEST_END  + i*windoe_size+test_size , [Y_COLS,RETURNS_COLS]]
...
def RunAllModels(Abs_train, Abs_test, Abs_validation ,X_train, Y_train, X_test, Y_test, X_validation,Y_validation, window_number):
	...
	RunRF(Abs_train, Abs_test, Abs_validation, X_train, Y_train, X_test, Y_test,X_validation,Y_validation,'RF_', window_number)
	#s = s + p
		#s1 = s1 + q
...
def RunRF(Abs_train, Abs_test, Abs_validation, X_train, Y_train,  X_test, Y_test, X_validation, Y_validation, name, window_number):
	#range = [1000]
	min_s_s_array = []
	min_s_l_array = [] 
	max_depth_array = []
	...
	min_samples_split_range = range(2,10) + [20,30,50,100]
	min_sample_leaf_range = range(1,12)
	max_depth_range = [5,10,15,25,40,50,70] 
	for i in min_samples_split_range:
		for j in min_sample_leaf_range:
			for k in range(3,4):
				model = RandomForestClassifier(n_estimators=300, criterion='gini', max_depth=None, min_samples_split=i, min_samples_leaf=j, 
								...
								oob_score=False, n_jobs=3, random_state=None, verbose=0, warm_start=False, class_weight=None)
			...
			#putting condition over plus accuracy and minus accuracy
				if  (accuracy[2][0] > 50) and (accuracy[0][0] < 65 )  :
				
					returns 		= ComputeReturns(Abs_test, Abs_train, pred_test, pred_train, Y_test, Y_train, name,0)
				
					min_s_l_array.append(j)
					min_s_s_array.append(i)
					max_depth_array.append(k)
	...
	min_s_l_array = np.asarray(min_s_l_array).T
	min_s_s_array = np.asarray(min_s_s_array).T
	max_depth_array = np.asarray(max_depth_array).T
	...
	if(len(predicted_test_acc_array) > 0):
		...
		m   = min_s_l_array[index_for_best_c_g]
		m_  = min_s_s_array[index_for_best_c_g]
		m__ = max_depth_array[index_for_best_c_g]

		print("best min sample split " + str(m))
		print("best min sample leaf " + str(m_))
		print("best max depth " + str(m__))


		X_train = np.vstack((X_train,X_test))
		Y_train = np.hstack((Y_train,Y_test))
		Abs_train = np.hstack((Abs_train,Abs_test))
		model = RandomForestClassifier(n_estimators=300, criterion='gini', max_depth=None, min_samples_split=m_, min_samples_leaf=m, 
								min_weight_fraction_leaf=0.0, max_features='auto', max_leaf_nodes=None, min_impurity_decrease=0.0, min_impurity_split=None, bootstrap=True, 
								oob_score=False, n_jobs=3, random_state=None, verbose=0, warm_start=False, class_weight=None)
	...
	#
	#out = np.vstack((actual_dist_array,predicted_train_array,predicted_test_array,predicted_train_acc_array,predicted_test_acc_array,ret_pt_tot_train,ret_pt_cor_inc_train,ret_pt_tot_test,ret_pt_cor_inc_test))
